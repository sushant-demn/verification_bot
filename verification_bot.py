...
async def on_member_join(member):
    try:
        ...
        if not sent:
            await member.send("❌ Failed to send verification email. Please try again later.")
            return
        ...
        if code_msg.content.strip() == verification_codes.get(member.id):
            ...
            if guild:
                ...
                if role:
                    ...
                    await member.send("⚠️ 'Verified' role not found. Contact an admin.")
            else:
                await member.send("❌ Server not found.")
        else:
            await member.send("❌ Incorrect code. Please try again.")
